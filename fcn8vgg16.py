...
class FCN8_VGG16:
    ...
    def train2(self, sess,
               epochs, batch_size,
               get_batches_fn, n_samples,
               keep_prob_value, learning_rate,
               ckpt_dir=None, summaries_dir=None):
        """
        This function is a copy of the original train() designed to work with batch generation and coordination using tensorflow libraries and not pure python. The idea is that
        this approach could be leverage tensorflow CPU and GPU load balance functions.
        """
        # restore from checkpoint if needed
        if ckpt_dir is None:
            saver = None
        ...
        for epoch in range(epochs):
            ...
            for i_batch in batches_pbar:
                images, labels = get_batches_fn(sess)
                feed_dict = {self._images: images,
                             self._labels: labels,
                             self._keep_prob: keep_prob_value,
                             self._learning_rate: learning_rate}
